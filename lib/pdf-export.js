...
// 目录的 file:// URL（结尾带 /）——给 <base href> 用。
// Markdown 导出 PDF 时正文是渲染到临时目录的，必须把 base 指回 md 原目录，
// 否则文档里的相对图片在 file:// 下全部取不到。
function dirFileUrl(absDir) {
  const url = fileUrl(absDir);
  return url.endsWith('/') ? url : url + '/';
}

// ---------- 判断 PDF 是否已经写完 ----------
//
// 为什么需要它：Chrome `--headless=old --print-to-pdf` 把文件写完之后**不会自己退出**。
// 实测（macOS / Chrome）三种文档：PDF 分别在 2.5s / 3.1s / 7.3s 落地，而进程一直挂着
// 直到被硬超时 SIGKILL —— 也就是说每次导出都要白等到超时上限，哪怕 PDF 早就好了。
//
// 判据用 `%%EOF`（PDF 规范要求的文件结束标记，只出现在文件末尾）而不是只看文件大小：
// 实测 Chrome 是一次性写入的（文件首次出现的那一刻尾部就已经有 %%EOF），但万一某个
// 版本或平台改成分段写，中间态不会带 EOF，所以这个判据不会把半成品当成写完的。
// 再叠一次"大小与上一拍相同"作保险。
const PDF_TAIL_BYTES = 1024;
function readPdfState(filePath) {
  let fd = null;
  try {
    fd = fs.openSync(filePath, 'r');
    const stat = fs.fstatSync(fd);
    if (stat.size < 64) return { size: stat.size, complete: false };
    const len = Math.min(PDF_TAIL_BYTES, stat.size);
    const buf = Buffer.alloc(len);
    fs.readSync(fd, buf, 0, len, stat.size - len);
    return { size: stat.size, complete: buf.includes('%%EOF') };
  } catch {
    return null;   // 还没创建 / 正在被写、读不到都算"还没好"
  } finally {
    if (fd !== null) { try { fs.closeSync(fd); } catch {} }
  }
}

// 先 SIGTERM 给它一个收尾的机会，没走就 SIGKILL。
// 走 SIGTERM 是为了让 Chrome 自己释放 user-data-dir 里的锁——虽然那是每次新建的临时
// 目录、我们随后就整个删掉，但强杀留下的半个 profile 偶尔会拖慢下一次启动。
const GRACEFUL_KILL_WAIT_MS = 1200;
...
async function _doExportPdf({ htmlPath, fileName }, onPhase) {
  ...
  return new Promise((resolve) => {
    ...
    // 监测 PDF 文件的状态。两件事：
    //   ① 文件出现 = 进入"写入"阶段，给前端一个阶段提示
    //   ② 文件写完（尾部有 %%EOF 且大小不再变）= chromium 的活干完了，主动收掉它。
    //      不这么做的话它会一直挂到下面那个硬超时，每次导出都白等二十多秒。
    let lastSize = -1;
    let stopRequested = false;
    let gracefulTimer = null;
    const writePoll = setInterval(() => {
      const st = readPdfState(outPath);
      if (!st) return;
      if (!writingEmitted) {
        writingEmitted = true;
        emit({ phase: 'writing', message: '正在写入 PDF…' });
      }
      if (!stopRequested && st.complete && st.size > 0 && st.size === lastSize) {
        stopRequested = true;
        clearInterval(writePoll);
        gracefulTimer = stopChild(child);
      }
      lastSize = st.size;
    }, 150);
    // 硬超时：渲染真的卡住了（页面死循环、等一个永远回不来的请求）时的兜底。
    // 正常路径已经由上面的主动收尾接管，走到这里说明 PDF 始终没写完。
    ...
    child.on('exit', (code) => {
      ...
      // 主动收尾（stopChild）走的也是这条路：它同样是被信号结束的。
      //
      // complete 只作为附加信息返回、不参与成功判定：正常路径的收尾条件本身就是
      // "尾部已有 %%EOF"，所以它必然为 true；只有走到硬超时（渲染卡死）才可能拿到
      // 一份截断的 PDF。不拿它否决成功是为了不引入回归——万一某个平台的 Chrome
      // 在 PDF 末尾追加了什么导致判据失效，也不该让本来能用的导出变成报错。
      const st = readPdfState(outPath);
      const size = st ? st.size : 0;
      const ok = size > 0;
      if (ok) {
        resolve({
          ok: true, savedPath: outPath, browser: browser.name, size,
