...
async function runInit({ existingConfig = null, force = false } = {}) {
  ...
  const config = {
    ...
    // 文档类型：HTML 与 Markdown 可共存，默认两者都扫，可在 Dashboard 设置里勾选
    docTypes: (existingConfig && Array.isArray(existingConfig.docTypes) && existingConfig.docTypes.length)
      ? existingConfig.docTypes
      : ['html', 'md'],
  };
