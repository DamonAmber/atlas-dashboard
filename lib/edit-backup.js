...
// 备份文件的扩展名跟随源文件。
// 原来硬编码 .html，于是 notes.md 被备份成 notes-a1b2c3d4-1699.html——
// 手工恢复时既看不出这是 Markdown，编辑器还会按 HTML 打开。
function backupExt(absPath) {
  const ext = path.extname(absPath);
  return /^\.[A-Za-z0-9]{1,12}$/.test(ext) ? ext : '.bak';
}
...
function backup(absPath) {
  ...
  const ts = Date.now();
  const dest = path.join(dir, `${stem}-${sha8}-${ts}${backupExt(absPath)}`);
...
// 只按 stem 前缀筛（不再限定 .html 后缀），否则 .md 的备份永远淘汰不掉
function pruneOld(dir, stem) {
  let entries;
  try {
    entries = fs.readdirSync(dir).filter(f => f.startsWith(stem + '-'));
  } catch {
    return;
  }
