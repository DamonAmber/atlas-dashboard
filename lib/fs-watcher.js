...
// 且此时 spawn 正常）。
//
// ## 为什么这个 bug 能一直藏着：它是 macOS / Windows 特有的
//
// 每个监听在不同平台上的代价完全不同：
//
//   · macOS：kqueue，**每个被监视的目录都要占一个真实 fd** → 目录一多就撞
//     RLIMIT_NOFILE。这是问题的发生地。
//   · Windows：每个目录一个句柄，同理。
//   · Linux：inotify 用的是 watch descriptor，而 libuv 让整个事件循环**共享
//     一个 inotify fd** —— 监听几千个目录也只占那一个。所以 Linux 上逐目录
//     监听从来不会耗尽 fd（它的天花板是 `/proc/sys/fs/inotify/max_user_watches`，
//     超了报 ENOSPC，是另一回事）。
//
// 也就是说 **CI（Linux）永远复现不了这个 bug**，而用户的 mac 上必现。
// 0.19.0 之前它就是这么躲过去的。改动之后 CI 实测 fd 净增 0、mac 上也是 0，
// 两边都不再有这个风险。
//
// ## 各平台走哪条路
//
//   · macOS / Windows：`recursive` 由内核提供（FSEvents / ReadDirectoryChangesW），
//     整棵树一个监听、0 个额外 fd。**这是本次修复真正解决的场景。**
//   · Linux + Node 20.13+：`recursive` 可用，但是 Node 在 JS 层逐目录建 watch
//     模拟出来的；因为上面说的 inotify 特性，fd 依然接近 0。
//   · Linux + Node 18/19、以及其它平台：探测不到 recursive，回退 chokidar，
//     行为与 0.18.x 完全一致。
//
// 模块导出的 SYSTEM_RECURSIVE 表示"递归是不是内核给的"。它只用来解释性能特征
// 与测试断言的适用范围，不影响功能。
//
// ## 原生后端要自己补齐的四件事
//
...
// 递归是内核提供的（macOS FSEvents / Windows ReadDirectoryChangesW），
// 还是 Node 在 JS 层逐目录模拟的（Linux）。只影响性能特征的解释与测试断言的
// 适用范围——注意"JS 层模拟"在 Linux 上同样不耗 fd（inotify 共享一个 fd），
// 所以这个标志不等于"这里省不了 fd"。
...
// 回退实现：探测不到 recursive 的平台走这条（Linux + Node 18/19 等）。
// 行为与 0.18.x 完全一致；而那些平台恰好都是 inotify 系的，逐目录监听
// 本来就不耗 fd，所以回退不会把 fd 问题带回来。
