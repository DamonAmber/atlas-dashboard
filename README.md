...
> 目录树（嵌套分组、拖拽重排）· 备注名 · 未读红点 · **收藏夹** · **标签筛选** · **和上次已读版本对比** · ⌘K 快速打开 · 桌面通知 · 在访达打开 · ⌘B 收侧栏 · iframe 平滑预览 · Markdown 实时预览与编辑。
...
- **收藏夹**：点文件行左侧的 ☆（或预览时用右上角的星标按钮）即收藏，侧栏「收藏」区把散落在各个分组里的常看文档聚到一处，按最近收藏排序，跨文件夹一眼可达。可折叠，✕ 取消收藏。
- **标签 + 按标签筛选**：hover 文件 → 🏷 → 逗号分隔输入（`周报, 待评审, AI`），最多 12 个。侧栏出现标签筛选条（按用量排序、带计数），点一下只看该标签的文档，**选多个标签是"同时具备"**，且与搜索、"仅未读"叠加生效。清空输入并确定即删除全部标签。
- **备注名**：hover 文件 → ✎ → 起一个你能记住的名字。原文件名不变。
- **重命名文件**：hover 文件 → `Aa` → 直接改磁盘上的真实文件名（备注、已读状态、收藏、标签、分组位置都会跟着迁移）。
