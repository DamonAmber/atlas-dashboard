...
> **首页未读概览** · 目录树（嵌套分组、拖拽重排）· 备注名 · 未读红点 · **收藏夹** · **标签筛选** · **和上次已读版本对比** · ⌘K 快速打开 · 桌面通知 · 在访达打开 · ⌘B 收侧栏 · iframe 平滑预览 · Markdown 实时预览与编辑 · **深浅色主题可固定**。
...
- `store.json` — 你拖拽形成的虚拟分组、备注名、已读时间、收藏、标签、分享链接、对比底本的索引
...
- **首页概览**：没打开文档时，右侧是一屏「今天该看什么」——文档总数 / 未读 / 项目数 / 收藏数四个数字，加上按修改时间倒序的**待看队列**、最近打开、收藏三张卡片，点一行直接进文档，不用去目录树里找红点。
...
- **目录树**：按你扫描根下的一级目录自动归类。可拖拽重排、新建嵌套分组、双击重命名分组。「全部文档」标题栏右侧有**全部折叠 / 全部展开**（一个按钮两个状态），分组多时不用一个个点。
...
- **未读红点**：基于文件 `mtime` 检测。点开预览即清除；右上角有"标为未读"和"全部已读"。已归档的分组不参与未读统计——树里看不见的文档不会再挂在底栏那行数字上。
