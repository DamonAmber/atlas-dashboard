...
- **标签 + 按标签筛选**：hover 文件 → 标签按钮 → 逗号分隔输入（`周报, 待评审, AI`），最多 12 个。侧栏出现标签筛选条（按用量排序、带计数），点一下只看该标签的文档，**选多个标签是"同时具备"**，且与搜索、"仅未读"叠加生效。清空输入并确定即删除全部标签。
...
除 Markdown 编辑器那组之外，**焦点落在预览的文档正文里时这些键同样有效**——
预览是独立文档，键盘事件本来传不到外壳，Atlas 会把这些和弦转发过来。
