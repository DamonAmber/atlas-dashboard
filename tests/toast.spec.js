// 扫描根增删的 toast 反馈（success / info / error + 自动消失）
//
// 本测试会增删扫描根，因此自起一个隔离的 Atlas 实例（独立 ATLAS_HOME + 临时扫描根
// + 随机端口），绝不触碰用户真实的 ~/.atlas/config.json。
// 早先版本直连用户本机 :4321 并直接改真实 config，中断时会残留临时扫描根，
...
(async () => {
  const atlas = await startAtlas({
    prefix: 'atlas-toast-spec-',
    files: {
      'proj/index.html': '<!doctype html><html><body><h1>fixture</h1></body></html>',
    },
  });
  // 待添加的扫描根：实例临时目录下的空目录，随实例一起销毁
  const testRoot = atlas.makeDir('to-be-added');
  ...
  try {
    ...
    // ===== 6. 全程只动了隔离实例自己的 config =====
    console.log('\n[6] 隔离性');
    const cfg = atlas.readConfig();
    check('实例 config 的扫描根仍都在实例临时目录内',
      cfg.scanRoots.every(p => p.startsWith(atlas.tmpDir)),
      JSON.stringify(cfg.scanRoots));
  } finally {
    await browser.close();
    await atlas.stop();   // kill 实例 + 删整个临时目录
  }
