...
| `npm run app:build` → **`codesign ... The timestamp service is not available`**（退出码 1，签某个 `locale.pak` 时失败） | Apple 的安全时间戳服务（`codesign --timestamp` 联网访问 `timestamp.apple.com`）**瞬时不可用**，不是证书/凭据/代码问题。**直接重跑** `source electron/build/notarize.env && npm run app:build` 即可，一般第二次就过（electron-builder 用 `--force` 重签，不用清理）。0.24.1 发版时遇到，重试一次即成功。 |
...
| **分开单独传 zip/dmg 仍然卡**：`gh release upload` 和直接 `curl` 到 `uploads.github.com` 都传不完，Release assets 长时间只剩 `latest-mac.yml` | 本机→GitHub 上行**网络抖动**期，单个 124MB 也传不完（而同一时段 `notarytool` 传 Apple 却很快，说明是到 GitHub 这条路的问题）。两条上传**并行**时每条被挤到 ~27KB/s，且长连接（~40 分钟）会被对端掐断：`curl: (55) Recv failure: Connection reset by peer`（实测 zip 传到 64MB、dmg 到 70MB 就断）。**诊断**：前台 `curl --max-time 100 … -o /tmp/x -w "uploaded=%{size_upload} speed=%{speed_upload}"`，看这 100s 到底传了多少字节 / 平均速度，据此区分"慢"还是"完全卡死"。**处置**：改成**单连接、串行、一次传完、中途绝不打断**——GitHub 资产上传**不支持断点续传**，一旦中断（含命令超时被杀、`Ctrl-C`）就得从 0 重来，反复打断只会永远传不完；同名残片会占住 asset 名，重传前先 `gh release view --json assets` 确认没有半成品（有就先删）。单连接通常能跑满带宽（本次网络恢复后 zip 19.5s、dmg 43.7s 就传完了）。若持续极慢，就等网速恢复再一次性传，别并行、别中断。**另注意**：用 `pgrep -fl` / `ps` 排查上传进程会把命令行里的 `gh` token 明文打出来——构造 `curl` 时用 `TOKEN=$(gh auth token)` 引用变量、不要把 token 写进会被别人看到的地方；万一泄露，`gh auth refresh` 轮换。0.29.2 发版时遇到。 |
...
- **0.29.3** (2026-09-17) — 修复 **Markdown 表格单元格里 GFM 合法的转义竖线 `\|` 被误当列分隔符**（纯前端 `public/vendor/markdown.js`）。用户反馈：表格某格写 `副标题「**房间名 \| 故障描述**」` 时，渲染出来加粗失效（`**` 按字面显示）、文字像被截断、该格之后的列也错位 / 丢失。根因：切列函数 `splitRow` 用 `s.split('|')` 无脑按每个竖线切分，把单元格内 GFM 用来表示「字面竖线」的转义写法 `\|` 也当成了列分隔符——于是该格从 `\|` 处被切成两段、凭空多出一列，而正文渲染只按表头列数 `headerCells.map` 取值、多出来的尾列被丢弃；同时 `**...**` 因收尾跑进被丢弃的片段、找不到配对而按字面显示。这还是个**往返 bug**：序列化侧 `cellText` 一直把单元格内的 `|` 转义成 `\|` 写回源码（0.29.0 引入），旧解析读不回，编辑保存后对不上。① **修复**：`splitRow` 改为手写扫描状态机（不用后行断言 `(?<!\\)`，沿用本文件为兼容老浏览器一贯回避 lookbehind 的约定，与 `parseCsv` 同风格），只在**未转义**的竖线处切列、把 `\|` 还原成字面 `|`，并正确处理 `\\|`（转义反斜杠 + 真分隔符）这类边界；行尾边框竖线造出的空尾列照旧去掉。② **验证**：新增 `tests/md-table-escaped-pipe.spec.js`（13 项：转义竖线不切列 + 加粗闭合 + 后续列不丢、一格多个转义竖线、表头里的转义竖线，以及普通表格 / 无边框表格回归），并加入 `npm test`；`md-render-and-roundtrip`、`md-table-edit`、`md-table-drag-persist` 回归全绿。③ **npm 与 App**：纯 `public/` 前端修复，npm 与桌面 App 同源生效，但已装 App 需重新出 **DMG + zip + latest-mac.yml** 才能拿到。
- **0.29.2** (2026-09-15) — 修复**只读预览页左侧「目录（TOC）」导航栏里标点被双重转义**（纯前端 `public/vendor/markdown.js`）。用户反馈：打开文档后，左侧目录里标题带的双引号显示成字面 `&quot;`（`&`、`<`、`>`、`'` 同样受影响）。根因：`renderPage` 的目录项文本由 `extractHeadings` 从**已渲染、已 `escapeHtml` 过**的正文 HTML 中用 `stripTags` 抽取——`stripTags` 只删标签、不解码实体，于是 `items[].text` 里存的是含 `&quot;` 字面的字符串；`tocTreeHtml` 随后又对它 `escapeHtml` 一次，形成双重转义（`&quot;` → `&amp;quot;`），浏览器解码一层后目录里就显示成 `&quot;`。正文标题因为直接保留原始 inner、由浏览器解码，显示一直正常，所以只有目录项 / 锚点 slug 出错（slug 里还会混入 `quot`/`amp` 残字）。① **修复**：新增与 `escapeHtml` 严格互逆的 `decodeEntities`（只解 `&amp;`/`&lt;`/`&gt;`/`&quot;`/`&#39;` 这 5 个 escapeHtml 会产出的实体，单次从左到右替换，保证原文里字面的 `&lt;`——被 escapeHtml 写成 `&amp;lt;`——不会被误解码回 `<`），`extractHeadings` 抽取标题时改用 `decodeEntities(stripTags(inner))` 得到干净纯文本，目录项文本与锚点 slug 都基于纯文本、`tocTreeHtml` 的那一次 `escapeHtml` 得到正确结果。② **验证**：`md-render-and-roundtrip`、`folder-reveal-and-toc-resize` 两套回归全绿；另用临时脚本核对含 `" & < '` 的多级标题，目录只做一次转义、锚点 slug 无实体残字。③ **npm 与 App**：纯 `public/` 前端修复，npm 与桌面 App 同源生效，但已装 App 需重新出 **DMG + zip + latest-mac.yml** 才能拿到。
- **0.29.1** (2026-09-11) — 修复 **0.29.0 的表格拖行在「引用块 / 列表项内表格」上保存不生效**（`public/app.js` + `public/vendor/markdown.js`）。用户反馈：md 编辑分栏里拖动表格整行，右侧预览确实重排了，但左侧源码没同步、保存后弹回原样。根因是脏块跟踪的粒度是「顶层块」——`serializeBlock` 对带 `data-md-raw` 且未标 `data-md-dirty` 的顶层块原样吐回渲染时记下的旧源码；而拖行的 `onEnd` 只在 `<table>` 上标脏。顶层表格时 `<table>` 自己就是那个带 `data-md-raw` 的块，正常；但表格嵌在**引用块 / 列表项**里时，带 `data-md-raw` 的是外层 `blockquote` / `ul`，`<table>` 只是它的后代——外层块没标脏 → 原样吐回旧顺序 → 源码不同步、保存回退（顶层表格因此一直正常，隔离测试也测不出，是这个 bug 长期潜伏的原因）。① **修 app.js**：`onEnd` 改用新 helper `markMdRawBlockDirty(table)`——从 `<table>` 向上冒泡到最近的带 `data-md-raw` 的祖先并给它标 `data-md-dirty`（找不到则兜底标自己），顶层表格行为不变、嵌套表格的外层块现在会被正确标脏重序列化。② **修 markdown.js `serializeList`**：原来列表项里只处理内联内容 + 嵌套 `ul/ol`，`<table>` 这类块级子节点会被当内联拍平成一行文本、彻底毁掉表格结构；新增 `LIST_ITEM_BLOCK_TAGS`（`TABLE` / `PRE` / `BLOCKQUOTE`）识别，把列表项内的块级内容当独立块序列化、缩进对齐到列表项内容列后单独成段（引用块内表格走 `serializeBlock(BLOCKQUOTE)` 本就能正确递归，无需改）。③ **验证**：新增 `tests/md-table-drag-persist.spec.js`（顶层 / 引用块内 / 列表项内三种嵌套，拖行 → 源码同步 → 保存 → 读磁盘是新顺序 → 结构保真 → 磁盘内容能重新渲染成正确行序的表格，补上了 `md-table-edit` 只验 `#md-source`、从不读磁盘的盲区），并加入 `npm test`；md-render-and-roundtrip、md-table-edit、md-editor-ux、preview-live-edit、rich-render、md-sync-highlight 回归全绿。全套 spec 全绿。纯 `public/` 前端修复，npm 与桌面 App 同源生效，但已装 App 需重新出 **DMG + zip + latest-mac.yml** 才能拿到。
...
- **0.26.1** (2026-09-07) — 把 0.26.0 没彻底解决的**「目录」视图展开/收起卡顿**真正修好（纯前端 `public/app.js` + `public/styles.css`）。0.26.0 按"height 动画触发逐帧 reflow"优化，但用户反馈目录视图仍明显掉帧。用大深目录树（720 文件 / 22960 个 DOM 节点）做帧率探针实测，定位到**真正的根因**：折叠的子树以 `display:none` 常驻 DOM，展开时浏览器要对整棵已建好的子树做**一次性布局**，同步阻塞约 85ms、掉帧——跟动画用 height 还是 fade 无关。① **根因修复**：给目录视图的 `.folder-children` / `.file` 加 `content-visibility: auto` + `contain-intrinsic-size`（浏览器原生的"列表虚拟化"——只对视口内及邻近的行做布局 / 绘制，视口外的行用占位尺寸跳过渲染）。实测展开大目录的同步阻塞从 ~85ms 降到 <1ms、掉帧 1→0、最大帧 41ms→9ms（稳 60fps），滚动也更轻。`--h-row=28px` 作为文件行高度估值，`auto` 关键字首次渲染后记住真实高度、自我校正，滚动条不跳。只作用于目录视图；分组视图不加——它要拖拽重排，`content-visibility` 会影响命中判定。② **动效统一**：性能根因既已解决，就删掉 0.26.0 里为绕开性能给大子树留的两条退化路径（展开=纯 opacity 淡入、收起=瞬间切掉），让展开 / 收起在**任意子树大小、两个方向都统一走高度滑动**（展开再叠加子项错峰淡入）。之前大目录"展开淡入、收起啪一下消失"的不对称突兀感消除，现在收起也是平滑滑动收拢。实测统一滑动在超大目录上仍 0 掉帧、最大帧 ~9ms。③ **验证**：`node --check` 通过；folder-toggle-with-jitter / archive-and-collapse-all / tree-view-and-open / drag-hover-expand / sidebar / modal-close 等关键路径全绿。桌面自动更新链路（0.26.0 修好的）不受影响。

- **0.26.0** (2026-09-07) — 两项按用户反馈做的桌面 App 优化：**目录树展开/收起丝滑化** + **自动更新修复并新增「检查更新」按钮**。① **目录树动画丝滑**（纯前端 `public/app.js` + `public/styles.css`）：用户反馈"目录"视图展开/收起明显卡顿掉帧，而"分组"视图丝滑。根因不是两者动画不同——它们共用同一套 `animateFolderChildren` + CSS `height` 过渡；差别在数据形状：分组是浅的虚拟树（子树小），目录是磁盘真实层级镜像（深、整棵子树一次性渲进 DOM）。对又大又深的子树做逐帧 `height`（触发布局的属性）动画，展开瞬间要给整棵子树做一次 layout、随后 200ms 内下方成百上千节点每帧 reflow，于是掉帧。修法三处：`animateFolderChildren` 改为**尺寸自适应**——子树高度 > 900px（约一屏）时放弃逐帧改 height，改用合成器代价恒定的 `opacity` 淡入（展开）/ 直接切换（折叠），节点再多都不 reflow；小/中子树保留原 height 过渡（就是分组丝滑那条路径）。CSS 给动画容器加 `contain: layout paint` + `will-change: height`，并给目录视图（只读、无拖拽）的每个 `.folder` 加 `contain: layout`，展开/折叠时下方兄弟节点只需整体平移、不必深度重排；`is-opening` 逐项入场动画封顶到前 14 个，避免几百个子项同时跑动画拖累合成器。分组视图刻意不加 containment（拖拽重排依赖正常布局流）。② **自动更新版本号修复**（`electron/server-manager.js` + `main.js` + `preload.js` + 前端）：用户反馈"桌面 App 升到 0.25.0 后版本号仍显示 0.24.1"。根因：桌面 App 启动时会复用已在跑的旧 CLI 守护进程（`atlas start`），窗口加载的是旧 server 的页面，`/api/config` 返回旧 `pkg.version`。修法：`server-manager` 复用前先 `GET /api/config` 校验运行中实例的版本，与本 App（`app.getVersion()`）不一致则不复用、改起 App 自带的 bundled server（自动挑新端口），保证跑的、显示的都是当前版本；设置面板的版本号桌面版改用 `app.getVersion()` 这个权威值显示。③ **新增「检查更新」按钮**（`electron/main.js` 加 `ipcMain` 更新桥 + 转发 `autoUpdater` 事件、`preload.js` 暴露 `updates.{getState,check,quitAndInstall,onStatus}` 与 `appVersion`、前端设置面板加控件）：此前桌面版只有静默的后台自动更新、无手动入口。现在设置里可**主动检查更新 → 显示下载进度 → 下载完按钮变「重启以更新」一键重启安装**（走 electron-updater 的 `quitAndInstall`，Squirrel.Mac 替换 bundle 后重启，版本随即变新）。用户主动检查触发的更新交给这个 in-app UI，后台自动检查到的仍弹原生框，避免重复打扰；开发/未打包环境提示"不支持自动更新"。④ **修复 release 流水线资产缺失**（`.github/workflows/release-app.yml`）：electron-updater 在 macOS 上做自动更新依赖 Release 里的 `Atlas-mac-arm64.zip` + `latest-mac.yml`（DMG 仅供手动下载），而原 workflow 只上传 DMG——这会导致自动更新/「检查更新」永远发现不了新版本。现改为三件资产都上传。⑤ **验证**：4 个改动的 JS 文件 `node --check` 通过；完整 `npm test`（35 个 spec）全绿，含 folder-toggle-with-jitter / tree-view-and-open / archive-and-collapse-all / drag-hover-expand / modal-close / sidebar 等关键路径。

- **0.25.0** (2026-09-07) — 两项按用户反馈做的优化：**侧栏「分组 / 目录」双视图** + **桌面 App 注册为文件的「打开方式」**。动机来自用户反馈表：① 侧栏"只有一级文件夹，二级 / 三级子目录看不到，一堆重复的 README 分不清"；② 在访达里双击 / 右键「打开方式」里没有 Atlas，选「其他…」时 Atlas 还是灰的。**① 目录树双视图**（前端 `public/` 三件套 + 后端 `server.js` 只加一个只读接口）：`server.js` 的 `makeRecord()` 只用 `relPath` 的第一段当 `projectName`、`reconcile()` 只建一级虚拟分组，深层目录一律被压平——这套虚拟分组机制（`autoFor` / `folderAliases` / 归档 / 空分组回收 / 拖拽自定义排序）耦合很深，直接改成多级树回归面太大。所以**不动它**，改为在「全部文档」标题行加一个 `分组 / 目录` 切换（`state.viewMode`，`localStorage: atlas:viewMode` 持久化）：**分组**视图＝原样（可拖拽、归档、手工子分组）；**目录**视图＝前端从 `state.files` 的 `relPath` **即时构建的真实多级目录树**（`buildDirTree()`，节点结构与虚拟分组树一致，故 `renderNode`/`countDescendants`/`nodeMatches`/`sortChildren` 全部复用），二级 / 三级子目录逐层展开、和访达里一样，是磁盘的**只读镜像**——不持久化、不挂 Sortable（`render()` 里按 `viewMode` 决定 `initSortables` 还是 `destroySortables`）、目录节点（`kind:'dir'`）只给折叠 +「在访达中显示」（走通用 `/api/reveal`，为此把它的守卫放宽到"扫描根自身"也放行），不给重命名 / 删除 / 新建子分组。另外**分组视图里深层文件（`relPath` 段数 > 2）在行末补一个中间目录提示**（`.file-dir-hint`），让平铺在同一分组下的多个同名文件（README、index……）也看得出各自在哪个子目录；hover 时和时间戳一样让位给操作按钮。折叠状态两个视图各记各的（目录节点 id 是 `dir:<rootIndex>/<相对目录>`，与虚拟分组的 `f-xxxx` 不撞）。**② 文件关联 +「用 Atlas 打开」**（`package.json` build + `electron/` + `server.js` + 前端）：`package.json` 的 `build` 加 `fileAssociations`（`md`/`markdown`/`html`/`htm`/`txt`/`text`/`csv`/`tsv`/`json`/`svg`，md 为 `Editor` 其余 `Viewer`，一律 `rank: Alternate`——只做"可选打开方式"，**不抢** md 的默认应用），electron-builder 据此写入 App 的 `CFBundleDocumentTypes`，Atlas 于是出现在访达「打开方式」里、不再置灰。行为层：`electron/main.js` 在 `will-finish-launching` 里注册 macOS `open-file`、`second-instance` 解析 argv（Windows / 已运行时双击）、`boot()` 读冷启动 argv，统一交给 `handleOpenPath` 缓存到 `pendingOpenPath`；窗口 `did-finish-load` 后 push 给前端，冷启动则由前端初始化时 `atlas:take-pending-open` 主动来取（取走即清、不重复打开）。`electron/preload.js` 新增 `onOpenPath` / `takePendingOpen` 两个桥。新增只读接口 `POST /api/resolve-open`：判断传入路径当前状态并返回 `ready` / `need-doctype` / `need-root` / `not-doc` / `not-found`；前端 `openExternalPath()` 据此处理——已在扫描根且类型启用就直接 `openFile`；类型没启用就问"是否启用并打开"；不在任何扫描根就问"是否把它所在目录加为扫描根后打开"——**全部经用户确认才动配置，不静默污染 `scanRoots`**；传入的是目录则切到目录视图打开。③ **同步 landing page（步骤 1.5）**：`docs/index.html` 的「嵌套目录树」卡改写为「目录树：分组 / 目录两种视图」、「顺手的细节」组新增「系统『打开方式』」卡、`#cur-version` 升到 0.25.0；README 的「目录树」段重写、新增「用 Atlas 打开文件（桌面 App）」条。④ **测试**：新增 `tree-view-and-open.spec.js`（17 项：分组视图平铺 + 深层文件路径提示、切到目录视图后多级目录展开 / 两个同名 README 各自可见 / 目录节点只读、视图选择持久化、`/api/resolve-open` 五种状态），已加入 `npm test`；全套 spec 全绿。⑤ **npm 与 App 的差异**：目录双视图、路径提示、`/api/resolve-open` 在 `public/` + `server.js`，npm 与 App 都拿得到；但 `fileAssociations`（文件关联）与 `open-file`/argv 接管在 `package.json` build 与 `electron/`，**只有桌面 App 生效**，所以本版**必须重新出 DMG + zip + latest-mac.yml 并传到 Release**，否则客户端用户（含自动更新）拿不到「打开方式」能力。

- **0.24.1** (2026-09-03) — 修复**编辑器右侧实时预览的文字色 bug**（纯前端，`public/vendor/markdown.js` + `public/app.js`；未动 `server.js` / `lib` / `electron`）。现象：把 Atlas 主题钉成与系统相反的档位时（如系统深色 + 应用钉浅色），编辑 md 时右侧「预览（可直接编辑）」面板白底配浅灰字、几乎看不清；反过来（系统浅色 + 应用钉深色）则深底配深字，都读不了。根因：预览正文 `.md-body` 的配色变量（`--md-fg` 等）在 `markdownCss` 里靠 `@media (prefers-color-scheme: dark)` 切换——它只读**系统**设置；而预览面板背景 `.md-preview-pane` 与左侧源码框 `.md-source` 走 `light-dark()` / `var(--text)`，跟随的是 Atlas 自己的强制主题（`:root[data-theme]`）。两套机制在"应用主题 ≠ 系统主题"时打架，前景色和背景色各走各的（左侧源码因走 `var(--text)` 一直正常，故只有右侧预览发灰）。iframe 只读预览页早有 `forcedThemeCss` 修这个割裂，但 dashboard 里的实时预览是主文档里的 `<div class="md-body">`、不是 iframe，`enhanceRich` 只渲染 Mermaid/KaTeX，没给它注入主题覆盖。① **修复**：`markdown.js` 新增并导出 `previewThemeCss`（`:root[data-theme="light"] .md-body{…mdVarsLight…}` 与 `:root[data-theme="dark"] .md-body{…mdVarsDark…}` 两条，复用既有配色变量）；`app.js` 的 `injectMarkdownCss()` 把它拼在 `markdownCss` 之后一起注入主文档。`[data-theme]` 祖先选择器特异性高于裸 `.md-body`，稳定压过 `@media`；主题为 system（无 `data-theme`）时两条都不匹配，仍由 `@media` 跟随系统，行为不变。② **验证**：Playwright 模拟四种「系统 × 应用」主题组合，逐一核对 `.md-body` 的计算 `--md-fg` 与文字色——钉浅色 → 近黑 `#1c1d21`、钉深色 → 浅 `#e7e9ec`、跟随系统两向均正确；全套 spec 全绿（含 `md-render-and-roundtrip` 的深色渲染用例）。③ **npm 与 App**：纯 `public/` 前端，npm 与 App 都拿得到，但已装的 App 需重新出 **DMG + zip + latest-mac.yml** 才能看到。

- **0.24.0** (2026-09-03) — **侧边目录树现代化重构**（纯前端，`public/` 三件套 + landing 两张特性卡；未动 `server.js` / `lib` / `electron`）。动机：旧侧栏虽然设计令牌已参考 Linear，但信息架构与视觉层级仍显旧——标签 / 收藏 / 最近三段同权重纵向堆叠、各一条灰线、长得一模一样，把真正的目录树挤到底部（"分区不清晰"）；文件夹与文件同字号同字重、行右侧还挂一列日期戳，像"上世纪办公软件的详细信息列表"；展开折叠是 `display:none` 瞬切、无动效。① **快捷区面板**（`public/index.html` 把 `#tag-bar`/`#fav-bar`/`#recent-bar` 包进新的 `.quick-zone`，重排为 收藏→最近→标签）：三段收进一个凹陷底色 + 发丝边 + 圆角的独立面板，段间极淡分隔线，一眼与下方"全部文档"树分开；空时用 `:has()` 整块隐藏、树独占剩余高度。**每段内部 id / 折叠态 / 显隐与持久化逻辑一律不变**（`renderFavorites`/`renderRecent`/`renderTagBar` 与各 `#*-toggle`、`atlas:favCollapsed` 等全部保留），所以既有行为测试零改动全绿。② **视觉层级**（`public/styles.css`）：文件夹图标加深、名字收紧字距，与文件常规字重拉开；**静息态不再显示右侧日期戳**（仅"内容命中"行保留时间戳 + 放大镜），去掉"表格列"观感；行内标签去边框、淡化底色。③ **展开折叠动效**（`public/app.js` 新增 `animateFolderChildren`，`toggleFolder` 单节点路径走它）：JS 驱动高度平滑动画 + 子项 stagger 淡入，尊重 `prefers-reduced-motion`；**折叠末态仍落到 `.collapsed` 类（=`display:none`）**，所以键盘导航过滤、"全部折叠"（走整树重建、瞬时无动画）、Playwright 可见性判定三条链路与过去完全一致。④ **顶层分组吸顶**（VS Code tree sticky scroll 同款）：`:where(.tree>.folder>.folder-header)` 0 特异性设表面色背景，深滚几百篇文档时顶层分组名吸顶不迷路（去掉 `.tree` 顶部 padding 消除吸顶残影）。⑤ **祖先路径高亮**：`markSidebarActive` 向上给祖先 `.folder` 加 `.on-active-path`，点亮当前文件所在层级链的缩进引导线。⑥ **同步 landing page（步骤 1.5）**：`docs/index.html` 的「嵌套目录树」「收藏与标签」两张特性卡补上吸顶 / 平滑折叠 / 快捷区分组的描述、`#cur-version` 升到 0.24.0。⑦ **测试**：全套 spec 全绿（favorites-and-tags 84 项、archive-and-collapse-all 34 项等侧栏相关无一改动即通过）。⑧ **npm 与 App**：纯 `public/` 前端，npm 与 App 都拿得到，但已装的 App 需重新出 **DMG + zip + latest-mac.yml** 才能看到。
